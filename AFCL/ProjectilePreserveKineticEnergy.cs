...
namespace AndrewFTW
{ 
    public class ProjectilePreserveKineticEnergy : MonoBehaviour
    {
        public BallisticProjectile parentRound;

		public List<bool> ConservesParentsEnergy;




		public float ParentRoundMass;
		public float InitialVelocity;

		public float ChildRoundMass;
		...
		public void Awake()
        {
			_parentRoundVel = InitialVelocity;
            Hook(); 
        } 
				...
				if (self.m_usesSubmunitions && !self.m_hasFiredSubmunitions)
				{
					...
					for (int i = 0; i < self.Submunitions.Count; i++)
					{
						...
						for (int j = 0; j < submunition.NumToSpawn; j++)
						{
							...
							float launchVel;
							if (ConservesParentsEnergy[i])
							{
								if (InitialVelocity < 10f)
								{
									launchVel = 10f;
								}
								else
								{
									// This is the condensed formula for the two kinetic energies being combined to solve the velocity of the child projectile
									launchVel = ( (float) (ChildRoundMass * 0.5 * Mathf.Pow(_parentRoundVel, 2) ) );	
								}
							}
							else
							{
