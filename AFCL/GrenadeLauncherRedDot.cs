    ...
    public class GrenadeLauncherRedDot : MonoBehaviour
    {
        ...
        private void CheckReticleVisibility()
        {
            ...
            else if (LensCollider != null)
            {
                //Debug.Log("aaaaa3");
                float distance = Vector3.Distance(this.gameObject.transform.position, GM.CurrentPlayerBody.Head.position) + 0.2f;
                //Debug.Log("aaaaa3a" + _muzzlePos);
                //Debug.Log("aaaaa3a" + ZeroDistances[CurrentZeroDistanceIndex]);
                Vector3 direction = _muzzlePos.position + this.transform.forward * ZeroDistances[CurrentZeroDistanceIndex] - _rightEye;
                //Debug.Log("aaaaa3b");
        ...
        public void Zero()
        {
            ...
            {
                //if (!IsIntegrated) AttachableFirearm = this.Attachment.curMount.Parent as FVRFireArm;

                if (IsIntegrated && AttachableFirearm == null) Debug.LogError("ScopeShaderZoom: FireArm not set on integrated Scope! Can't zero sight!");
                ...
                Vector3 p = AttachableFirearm.MuzzlePos.position + AttachableFirearm.GetMuzzle().forward * zeroDistance + AttachableFirearm.GetMuzzle().up * num;
